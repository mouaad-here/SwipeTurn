...
def build_user_profile_text(parsed_data: dict, cv_text: str) -> str:
    """
    Build compact user profile text for robust embedding quality.
    """
    ...
    return (
        f"Candidate profile. Skills: {skills_text}. Fields: {fields_text}. "
...
def build_user_profile_text_from_user(user: dict) -> str:
    """
    Build compact text from persisted user fields for query-time reranking.
    """
    ...
    return (
        f"Candidate profile. Skills: {', '.join(skills)}. "
...
def generate_cv_embedding(parsed_data: dict, cv_text: str) -> list:
    """
    Generates a 384-dimensional vector for a user's CV.
    Matches the format used in the job processing pipeline.
    Uses structured fields and reduced CV snippet to avoid noise.
    """
